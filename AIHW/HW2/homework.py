...
class MinMaxAlgo(object):
    ...
    def possible_actions(self, board, symbol, func_name):
        ...
        """
        if func_name == "max_value":
            stakes = sorted(stakes, key=itemgetter(3), reverse=True)
            raids = sorted(raids, key=itemgetter(3), reverse=True)
        else:
            stakes = sorted(stakes, key=itemgetter(3))
            raids = sorted(raids, key=itemgetter(3))
        """
        stakes.extend(raids)
        return stakes
        #return sorted(stakes, key=itemgetter(3), reverse=True)
...
def main():
    ...
    if read_input.algo_type == "MINIMAX":
        minmax = MinMaxAlgo(read_input.player_symbol, read_input.depth,\
