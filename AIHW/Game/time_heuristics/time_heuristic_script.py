...
class AlphaBetaAlgo(object):
    ...
    def write_to_output(self, move, board):
        with open("heuristic_output.txt", "w") as fp:
            fp.write(move+'\n')
            for row in board:
                fp.write(''.join(row))
                fp.write('\n')
...
def main():
    ...
    for i in xrange(2, 10):
        ...
        if len(actions)>12 and i == 9:
            continue

        if len(actions)>18 and i == 8:
            continue

        if len(actions)>25 and i == 7:
            continue

        if len(actions)>35 and i == 6:
            continue

        if len(actions)>55 and i == 5:
            continue
        ...
        print string_val,
        alphabeta.write_to_text_file("./time_heuristics.txt", string_val)
